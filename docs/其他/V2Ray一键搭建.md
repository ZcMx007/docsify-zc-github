...
论如何科学上网——之前的科学上网使用的是影梭（SSR），可惜现在已经不能用了，你需要频繁的更换端口号来防止被查封。所以现在谁还用SSR来科学上网呀？我选择V2Ray哈哈。V2Ray相比较SSR更稳定，需要频繁更换端口号的时代已经过去了。

本文章参考了：
注册域名搭建：<http://luyiminggonnabeok.cn/2019/10/18/%E4%B8%80%E9%94%AE%E8%84%9A%E6%9C%AC%E9%85%8D%E7%BD%AEV2ray%E6%A2%AF%E5%AD%90%E6%95%99%E7%A8%8B/>
一键搭建脚本：<https://www.itblogcn.com/article/406.html>

## 前期准备

1、购买非大陆Linux服务器(阿里的轻量应用服务器)
2、使用Linux连接工具（Xshell/其他）远程连接

## 正式搭建

此处我们采用的是**一键搭建脚本**原作者封装的脚本直接在服务器上运行：直接运行脚本```bash <(curl -s -L https://git.io/v2ray-setup.sh)```即可。

## 脚本解析

其实上面运行脚本成功后即表示服务端安装V2Ray已经成功，你只需要将部署成功后的的返回数据保存下来，在客户端配置完成即可科学上网了（如果失败，请参照上面的链接），但是授人以鱼不如授人以渔，万一挂了呢，因此我决定看下作者的脚本具体做了哪些事情。

脚本的意思很简单：就是使用bash命令执行脚本，脚本是解析重定向的一个资源路径，让我们跳转至该url资源路径:
...
我们观察到改脚本进行了控制台输出，而后又使用bash命令执行解析重定向的一个资源路径后的脚本文件，这才是脚本的核心所在：
...
关于脚本的解析会在精学Linux后做进一步解析。
Linux参考文章：
Linux之bash:<https://zhuanlan.zhihu.com/p/145430701>
Linux echo详解：输出命令<http://c.biancheng.net/view/934.html>
...
![](images/2021-07-06-14-09-34.png)
![](images/2021-07-06-14-10-05.png)

v2rayU for Mac

```https://rachel.heartango.com/V2rayU.dmg```

BifrostV for Android

```https://rachel.heartango.com/BifrostV_v0.6.8_apkpure.com.apk```

把刚才在电脑端导出的，形如vmess://开头的链接复制到手机里的url配置中即可
