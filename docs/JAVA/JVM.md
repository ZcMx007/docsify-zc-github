...
GC的四大算法：引用计数法、复制算法、标记整理算法、标记清除算法、分代收集算法。话说这不是五种算法么？其实不是的，分代收集算法其本质是复制算法和标记清除算法的结合算法，年轻代（GC回收频繁）和老年代（GC回收频率很小）实行的不一致的算法结合。具体查看<https://zhuanlan.zhihu.com/p/297965515>