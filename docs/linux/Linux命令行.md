...
ps -ef | grep ssh # ps 与grep 常用组合用法，查找特定进程以及其父进程 效果不怎么明显
...
其中的"|"就是linux中的管道符，它的作用是将左侧命令获得的结果作为右侧命令的输入进行计算获得结果 例如```ps -aux | sort -rnk 4```命令就是先将当前终端机的所有运行程序的进程信息列出，再通过sort命令将所有进程信息进行sort排序，最后展示经过双重处理后的结果;每多加一个"|"，就表示对结果数据多一重处理操作。
在linux中的">"和">>"也是十分重要的且功能性也非常强！">"是指将结果输出到指定的文件，而">>"不仅会将结果输出到指定的文件，而且还会向文件中追加当前日期。

如果要查找指定的应用的进程信息，那只需要使用```ps -aux | grep [应用名]```即可，使用```ps -ef | grep [应用名]```主要是为了额外获得进程信息的父进程号，但是这样的进程关系其实并不能被明显的观察到。如果要更加清楚的观察到类似的进程信息，则完全可以通过```ps -axjf | grep [应用名]```列出类似程序树的程序显示,或者直接使用```pstree -pu```命令直接列出程序树观察即可。
